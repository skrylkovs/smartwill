...
contract SmartWill {
    ...
    ISmartWillFactory public factory;
    ...
    constructor(
        ...
        factory = ISmartWillFactory(msg.sender);
    }

    function getOwnerLastActivity() public view returns (uint256) {
        return factory.getLastPingOf(owner);
    }
    ...
    function transferToHeir() external returns (bool) {
        ...
        uint256 ownerLastPing = getOwnerLastActivity();
        require(
            block.timestamp >= ownerLastPing + willActivateWaitingPeriod,
    ...
    function canTransferNow() external view returns (bool) {
        uint256 ownerLastPing = getOwnerLastActivity();
        bool ownerInactive = block.timestamp >= ownerLastPing + willActivateWaitingPeriod;
    ...
    function isOwnerActive() external view returns (bool) {
        uint256 ownerLastPing = getOwnerLastActivity();
        return block.timestamp < ownerLastPing + willActivateWaitingPeriod;
    }

    function getNextPossibleTransferTime() external view returns (uint256) {
        uint256 ownerLastPing = getOwnerLastActivity();
        uint256 activityExpiry = ownerLastPing + willActivateWaitingPeriod;
